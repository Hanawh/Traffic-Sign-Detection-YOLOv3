...
if __name__ == "__main__":
    ...
    parser.add_argument("--class_path", type=str, default="config/traffic.names", help="path to class label file")
    parser.add_argument("--conf_thres", type=float, default=0.95, help="object confidence threshold")
    parser.add_argument("--nms_thres", type=float, default=0.05, help="iou thresshold for non-maximum suppression")
    ...
    for batch_i, (img_paths, input_imgs) in enumerate(dataloader):
        ...
        # Get detections
        with torch.no_grad():
            detections = model(input_imgs) # torch.Size([1, 64512, 10])
    ...
    for img_i, (path, detections) in enumerate(zip(imgs, img_detections)):
        ...
        if detections is not None:
            ...
            for x1, y1, x2, y2, conf, cls_conf, cls_pred in detections:
                ...
                f = open("txt/{}.txt".format(name), "a")
                print("{} {} {} {} {} {}".format(name,classes[int(cls_pred)],int(x1),int(y1),int(x2),int(y2)), file=f)
                f.close()
                ...
                plt.text(
                    x1,
                    y1,
                    s="%s:%.2f" % (classes[int(cls_pred)], cls_conf),
                    color="red",
